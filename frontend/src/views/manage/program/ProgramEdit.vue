<template>
  <a-modal v-model="show" title="修改项目" @cancel="onClose" :width="800">
    ...
    <a-form :form="form" layout="vertical">
      <a-row :gutter="20">
        <a-col :span="12">
          <a-form-item label='项目标题' v-bind="formItemLayout">
            <a-input v-decorator="[
            'title',
...
export default {
  ...
  computed: {
    ...
    show: {
      get: function () {
        return this.programEditVisiable
      },
      set: function () {
      }
    }
  ...
  methods: {
    ...
    setFormValues ({...program}) {
      this.rowId = program.id
      let fields = ['title', 'content', 'publisher']
      let obj = {}
      Object.keys(program).forEach((key) => {
        if (key === 'images') {
          this.fileList = []
          this.imagesInit(program['images'])
        }
